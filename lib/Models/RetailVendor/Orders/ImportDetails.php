 ...
 *
 * @category Class
 * @description This section will contain import details for an import order.
 * @package  ClouSale\AmazonSellingPartnerAPI
 * @author   Swagger Codegen team
 * @link     https://github.com/swagger-api/swagger-codegen
 */
...
self::INTERNATIONAL_COMMERCIAL_TERMS_DELIVER_DUTY_PAID,        ];
     ...
     *
     * @param string $method_of_payment This is used for import PO's only. If the recipient requests, this field will contain the Shipment Method of Payment.
     *
     * @return $this
     */
     ...
     *
     * @param string $international_commercial_terms This is used for import POs only. incoterm (aka International Commercial Term) are used to divide transaction costs and responsibilities between buyer and seller and reflect state-of-the-art transportation practices..1) ExWorks - This term places the maximum obligation on the buyer and minimum obligations on the seller. The seller makes the goods available at its premises. The buyer is responsible for all costs of the transportation of the shipment and bears all the risks for bringing the goods to their final destination. 2) FreeCarrier - The seller hands over the goods, cleared for export, into the disposal of the carrier (named by the buyer). The buyer pays for all the additional costs of transportation and risk passes when the goods are handed over to the carrier. 3) FreeOnBoard -  Ocean shipments only. The seller must deliver the goods alongside the ship at the named port, and clear the goods for export. The buyer pays for all the additional costs of transportation and risk passes when the goods are alongside the ship. 4) FreeAlongSideShip - Ocean shipments only. The seller must load the goods on board the vessel, cleared for export. The buyer pays for all the additional costs of transportation and risk passes when the goods are loaded on the ship. 5) CarriagePaidTo -  The seller pays for transportation to the named port of destination, but risk transfers to the buyer upon handing goods over to the first carrier. The buyer pays for all destination charges. 6) CostAndFreight - Ocean shipments only. Seller pays for transportation to the named port of destination, but risk transfers to the buyer once the goods are loaded on the vessel. The buyer pays for all destination charges. 7) CarriageAndInsurancePaidTo -Seller pays for transportation and insurance to the named port of destination, but risk transfers to the buyer upon handing goods over to the first carrier. The buyer pays for all destination charges. 8) CostInsuranceAndFreight- Ocean shipments only. Seller pays for transportation and insurance to the named port of destination, but risk transfers to the buyer once the goods are loaded on the vessel. The buyer pays for all destination charges. 9) DeliveredAtTerminal- Seller pays for transportation up to the destination terminal, and risks up to the point that the goods are unloaded at the terminal. The buyer pays for import clearance, duties & taxes and delivery costs. 10)DeliveredAtPlace - Seller pays for transportation to the named destination, and risk transfers at the point that the goods are ready for unloading by the buyer. The buyer pays for import clearance, duties & taxes and delivery costs. 11) DeliverDutyPaid -Seller is responsible for delivering the goods to the named place in the country of the buyer, and pays all costs in bringing the goods to the destination including import duties and taxes. This term places the maximum obligations on the seller and minimum obligations on the buyer.
     *
     * @return $this
     */
     ...
     *
     * @param string $port_of_delivery This field is used for import POs. The port where goods on an import PO must be delivered by the vendor. This should only be specified when the internationalCommercialTerms is FOB. Free-text field, limited to 64 characters.
     *
     * @return $this
     */
     ...
     *
     * @param string $import_containers Types and numbers of container(s) for import POs. Can be a comma-separated list if shipment has multiple containers. HC signifies a high-capacity container. Free-text field, limited to 64 characters. The format will be a comma-delimited list containing values of the type: $NUMBER_OF_CONTAINERS_OF_THIS_TYPE-$CONTAINER_TYPE. The list of values for the container type is: 40'(40-foot container), 40'HC (40-foot high-capacity container), 45', 45'HC, 30', 30'HC, 20', 20'HC.
     *
     * @return $this
     */
     ...
     *
     * @param string $shipping_instructions Special instructions regarding the shipment. This field is used for import POs.
     *
     * @return $this
     */
